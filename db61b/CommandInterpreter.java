...
class CommandInterpreter {
    ...
    void selectStatement() {
        ...
        _input.next("from");
        Table table = tableName();
        if (_input.nextIs(",")) {    //select from two table//
            _input.next(",");
            Table table2 = tableName();
            if (!_input.nextIs("where")) { //select from two table with no condition
                Table result = table.select(table2, columnNames, null);
                System.out.println("Search results:");
                result.print();
            } else { // select from two table with conditions
                
            }

        } else {        // only select from one table
            if (!_input.nextIs("where")) { //This means that there is no conditions
                Table result = table.select(columnNames, null);
                System.out.println("Search results:"); 
                result.print();
            } else {   //This means there is one table with conditions
                ArrayList<Condition> conditionList = new ArrayList<>();
                conditionList = conditionClause(table);
                Table result = table.select(columnNames, conditionList);
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        ...
        while (!_input.nextIs(";")) {
            _input.next(",");
            Condition condition2 = condition(tables);
            conditionList.add(condition2);
        }
        return conditionList;        // REPLACE WITH SOLUTION
    }
    ...
    Condition condition(Table... tables) {
        ...
        Column column1 = new Column(column1name, tables);
        String relation = name();
        if (_input.nextIs(Tokenizer.LITERAL)) {
            String val2 = name();
            Condition condition = new Condition(column1, relation, val2);
            return condition;
        } else {
            String column2name = name();
            Column column2 = new Column(column2name, tables);
